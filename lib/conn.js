...
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, await, sleep, reSize } = require('./myfunc')
...
resolve(ab)
    ...
    conn.parseMention = function (text = '') {
           ...
           conn.react = async (jid, text, m, options) => {
           	if(text === 'wait') text = "⏳"
           if(text === 'done') text = "✅"
           	return conn.sendMessage(jid, { react: { text: text, key: m.key }, ...options }) 
           }
    ...
    conn.sendFooterText = async (jid, text, footer, quoted, options) => {
